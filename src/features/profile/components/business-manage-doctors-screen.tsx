...
import { deleteDoctor, getManagedDoctors, getOwnedBusinessInfo } from "../profile.service";
import type { ManagedDoctor } from "../catalog.types";

export function BusinessManageDoctorsScreen({ slug }: { slug: string }) {
  const [page, setPage] = useState(1);
  const [deleteTarget, setDeleteTarget] = useState<ManagedDoctor | null>(null);
  const queryClient = useQueryClient();
  const business = useQuery({ queryKey: ["businesses", "mine", slug], queryFn: () => getOwnedBusinessInfo(slug) });
  const query = useQuery({ queryKey: ["business", slug, "doctors", page], queryFn: () => getManagedDoctors(slug, page) });
  const remove = useMutation({ mutationFn: (doctorId: string) => {
    if (!business.data?.id) throw new Error("Business is still loading.");
    return deleteDoctor(business.data.id, doctorId);
  }, onSuccess: async () => { setDeleteTarget(null); await queryClient.invalidateQueries({ queryKey: ["business", slug, "doctors"] }); } });
  ...
  return <div className="min-h-dvh bg-slate-50 pb-10">
    <MobileHeader title="Doctors" subtitle={query.data?.business?.name || business.data?.name || "Loading business..."} />
    ...
    <DeleteSheet doctor={deleteTarget} pending={remove.isPending} onClose={() => setDeleteTarget(null)} onConfirm={() => deleteTarget && remove.mutate(deleteTarget.id)} />
  </div>;
}

function DoctorRow({ slug, doctor, onDelete }: { slug: string; doctor: ManagedDoctor; onDelete: () => void }) {
  const specialties = doctor.specialties?.slice(0, 2).map((specialty) => specialty.name || specialty.label).filter(Boolean).join(" | ") || "No specialty";
  const nextAvailable = doctor.schedule?.next_available;
  return <article className="border-b border-slate-100 p-4 last:border-b-0"><div className="flex items-center gap-3"><span className="flex size-14 shrink-0 items-center justify-center rounded-lg bg-brand-50 text-brand"><i className="fa-solid fa-user-doctor text-lg" aria-hidden="true" /></span><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><h2 className="truncate text-sm font-extrabold text-foreground">{doctor.name}</h2><p className="shrink-0 text-sm font-extrabold text-brand">{formatCurrency(doctor.consultation_fee)}</p></div><p className="mt-1 truncate text-xs font-semibold text-foreground-muted">{specialties}</p><p className="mt-1 truncate text-xs text-foreground-muted">{[doctor.qualification, nextAvailable].filter(Boolean).join(" | ")}</p></div></div><div className="mt-3 grid grid-cols-2 gap-2"><Link href={`/business/${encodeURIComponent(slug)}/manage/doctors/${encodeURIComponent(doctor.id)}/edit`} className="flex h-8 items-center justify-center gap-1.5 rounded-lg bg-brand-50 text-[11px] font-extrabold text-brand"><i className="fa-solid fa-pen" />Edit</Link><button type="button" onClick={onDelete} className="flex h-8 items-center justify-center gap-1.5 rounded-lg bg-red-50 text-[11px] font-extrabold text-danger"><i className="fa-solid fa-trash" />Delete</button></div></article>;
}

function DeleteSheet({ doctor, pending, onClose, onConfirm }: { doctor: ManagedDoctor | null; pending: boolean; onClose: () => void; onConfirm: () => void }) {
...
function formatCurrency(value: number | string) { return new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR", maximumFractionDigits: 0 }).format(Number(value)); }
