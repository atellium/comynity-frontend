...
export function BusinessManageDoctorsScreen({ slug }: { slug: string }) {
  ...
  return <div className="min-h-dvh bg-slate-50 pb-10">
    ...
    <main className="mx-auto w-full max-w-3xl px-page pt-5">
      ...
      {query.data && query.data.results.length > 0 && <><div className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-[0_2px_8px_rgba(15,23,42,0.04)]">{query.data.results.map((doctor) => <DoctorRow key={doctor.id} slug={slug} doctor={doctor} onDelete={() => setDeleteDoctor(doctor)} />)}</div>{pagination && pagination.total_pages > 1 && <nav className="mt-5 flex items-center justify-between gap-3" aria-label="Doctor pagination"><button type="button" disabled={!pagination.has_previous || query.isFetching} onClick={() => changePage(page - 1)} className="h-10 rounded-xl border border-slate-200 bg-white px-4 text-xs font-extrabold disabled:opacity-40"><i className="fa-solid fa-chevron-left mr-2" />Previous</button><span className="text-xs font-semibold text-foreground-muted">Page {pagination.page} of {pagination.total_pages}</span><button type="button" disabled={!pagination.has_next || query.isFetching} onClick={() => changePage(page + 1)} className="h-10 rounded-xl bg-brand px-4 text-xs font-extrabold text-white disabled:opacity-40">Next<i className="fa-solid fa-chevron-right ml-2" /></button></nav>}</>}
    </main>
