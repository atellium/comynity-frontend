...
import type { BusinessSection } from "../profile.types";
...
export function BusinessDashboardScreen({ slug }: { slug: string }) {
  ...
  return <div className="min-h-dvh bg-white dark:bg-white">
    ...
    <main className="mx-auto flex w-full max-w-3xl flex-col gap-3 px-page pt-5">
      <DashboardLink href={`/business/${encodeURIComponent(slug)}/manage/info`} icon="fa-building" title="Business Information" description="View your business details, contact information and hours." />
      <DashboardLink href={`/business/${encodeURIComponent(slug)}/manage/services`} icon="fa-screwdriver-wrench" title="Services" description="View and manage your services." />
      {enabledSections.map((section) => {
        const config = SECTION_LINKS[section];
        return <DashboardLink key={section} href={`/business/${encodeURIComponent(slug)}/manage/${config.path}`} icon={config.icon} title={config.title} description={config.description} />;
      })}
...
function DashboardLink({ href, icon, title, description }: { href: string; icon: string; title: string; description: string }) {
  return <Link href={href} className="flex items-center gap-3 rounded-2xl border border-slate-100 bg-white p-4 shadow-[0_2px_8px_rgba(15,23,42,0.04)]">
    <span className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-brand-50 text-brand" aria-hidden="true"><i className={`fa-solid ${icon}`} /></span>
    <span className="min-w-0 flex-1"><span className="block text-sm font-extrabold text-foreground">{title}</span><span className="mt-0.5 block truncate text-xs text-foreground-muted">{description}</span></span>
    <i className="fa-solid fa-chevron-right text-xs text-foreground-muted" aria-hidden="true" />
  </Link>;
}
