...
export function BusinessInfoScreen({ slug }: { slug: string }) {
  ...
  return <div className="min-h-dvh bg-slate-50 pb-10"><MobileHeader title="Business Information" subtitle={query.data?.name ?? "Loading business…"} /><main className="mx-auto w-full max-w-3xl px-page pt-5">{query.isPending && <InfoSkeleton />}{query.isError && <div role="alert" className="rounded-2xl border border-red-100 bg-red-50 p-4 text-sm text-danger"><p className="font-semibold">Couldn&apos;t load business information.</p><button type="button" onClick={() => void query.refetch()} className="mt-2 font-extrabold underline">Try again</button></div>}{query.data && <BusinessInfoCards business={query.data} onEdit={setEditing} onBusinessSaved={(updated) => queryClient.setQueryData(["businesses", "mine", slug], updated)} />}</main>{query.data && editing === "gallery" && <BusinessGalleryEditor business={query.data} onClose={() => setEditing(null)} onSaved={(updated) => queryClient.setQueryData(["businesses", "mine", slug], updated)} />}{query.data && editing && editing !== "gallery" && <BusinessInfoEditor key={editing} section={editing} business={query.data} onClose={() => setEditing(null)} onSaved={(updated) => queryClient.setQueryData(["businesses", "mine", slug], updated)} />}</div>;
}
...
function BusinessInfoCards({ business, onEdit, onBusinessSaved }: { business: OwnedBusinessInfo; onEdit: (section: BusinessInfoSection | "gallery") => void; onBusinessSaved: (business: OwnedBusinessInfo) => void }) {
  ...
  return <div className="space-y-4">
    ...
    <InfoCard title="Offerings" icon="fa-list-check" onEdit={() => onEdit("offerings")}><InfoRow label="Business offerings" value={business.offerings?.map((offering) => offering.trim()).filter(Boolean).join(", ")} /></InfoCard>
