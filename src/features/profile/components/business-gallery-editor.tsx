...
import { getBusinessGallery, updateBusinessGallery, uploadBusinessGalleryImages } from "../profile.service";
...
export function BusinessGalleryEditor({ businessSlug, businessName, onClose, onSaved }: { businessSlug: string; businessName: string; onClose: () => void; onSaved: () => void }) {
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      const uploaded = await uploadBusinessGalleryImages(businessSlug, pending.map((item) => item.file));
  ...
  async function addFiles(files: FileList) {
    ...
    try {
      ...
      const compressed = await Promise.all(selected.slice(0, Math.max(0, available)).map((file) => compressImage(file, { maxWidth: 1600, quality: 0.95 })));
