...
function OwnedBusinessCard({ business }: { business: OwnedBusiness }) {
  ...
  const publicPath = `/${encodeURIComponent(business.slug)}`;
  ...
  return <article className="bg-white p-3">
    <div className="flex min-w-0 gap-2.5">
      <div className="relative size-14 shrink-0 overflow-hidden rounded-lg bg-slate-100"><Image src={!imageFailed && image ? image : "/images/default.jpg"} alt="" fill sizes="56px" className="object-cover" onError={() => setImageFailed(true)} /></div>
    ...
    <div className="mt-2.5 grid grid-cols-3 gap-1.5 border-t border-slate-100 pt-2.5">
      ...
      <CardAction href={`/business/${encodeURIComponent(business.slug)}/manage/dashboard`} icon="fa-gear" label="Manage" />
