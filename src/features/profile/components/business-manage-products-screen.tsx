...
import type { OwnedProduct } from "../catalog.types";
import { deleteProduct, getOwnedProducts } from "../profile.service";

export function BusinessManageProductsScreen({ slug }: { slug: string }) {
  const [page, setPage] = useState(1);
  const [deleteTarget, setDeleteTarget] = useState<OwnedProduct | null>(null);
  const queryClient = useQueryClient();
  const query = useQuery({ queryKey: ["businesses", slug, "manage-products", page], queryFn: () => getOwnedProducts(slug, page) });
  const remove = useMutation({ mutationFn: (productSlug: string) => deleteProduct(slug, productSlug), onSuccess: async () => { setDeleteTarget(null); await queryClient.invalidateQueries({ queryKey: ["businesses", slug, "manage-products"] }); } });
  const businessName = query.data?.business?.name?.trim();
...
function getProductImage(product: OwnedProduct) {
  const image = product.images?.[0];
  return image?.upload.url ?? null;
}

function formatPrice(product: OwnedProduct) {
  if (product.price_type === "ask") return "Ask price";
  const price = product.price ? formatCurrency(product.price) : "";
  if (product.price_type === "range" && product.max_price) return `${price} - ${formatCurrency(product.max_price)}`;
  if (product.price_type === "starts_from") return `From ${price}`;
  return price || "No price";
}

function formatCurrency(value: string) {
  const amount = Number(value);
  return Number.isFinite(amount) ? new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR", maximumFractionDigits: 0 }).format(amount) : value;
}

function ProductListSkeleton() {
  return <div className="overflow-hidden rounded-2xl border border-slate-100 bg-white" aria-label="Loading products">{[0, 1, 2, 3].map((item) => <div key={item} className="flex animate-pulse justify-between border-b border-slate-100 p-4"><div className="w-2/3"><div className="h-4 w-3/4 rounded bg-slate-100" /><div className="mt-2 h-3 w-1/2 rounded bg-slate-100" /></div><div className="h-4 w-16 rounded bg-slate-100" /></div>)}</div>;
}
