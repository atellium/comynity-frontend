...
export function usePopularCategories() {
	return useQuery({
		...
		select: (categories) =>
			categories.filter((category) => isPopularVisible(category) && category.is_popular),
		staleTime: CATEGORY_COLLECTION_CACHE_TIME,
		gcTime: CATEGORY_COLLECTION_CACHE_TIME,
	});
}

export function useBusinessCategoriesByParent(parentSlug: string) {
	return useQuery({
		queryKey: ["categories", "business", "parent", parentSlug],
		queryFn: ({ signal }) => getBusinessCategoriesByParent(parentSlug, signal),
		enabled: parentSlug.trim().length > 0,
		select: (categories) =>
			categories.filter((category) => isPopularVisible(category)),
		staleTime: CATEGORY_COLLECTION_CACHE_TIME,
		gcTime: CATEGORY_COLLECTION_CACHE_TIME,
	});
}
