...
type DemoBusinessContext = {
  ...
  handle: string;
  slug: string;
  media: { thumbnail: string | null; cover_image?: string | null };
  location: {
    locality: string;
...
export function getDemoProductBySlug(slug: string, business?: BusinessNameDetail): ProductDetail | null {
  ...
  const listProduct = materializeDemoProduct(template, businessContext);
  return {
    id: listProduct.id,
    name: listProduct.name,
    slug: listProduct.slug,
    short_description: listProduct.short_description,
    price_type: listProduct.price_type,
    price: listProduct.price,
    max_price: listProduct.max_price,
    original_price: listProduct.original_price,
    is_featured: listProduct.is_featured,
    sort_order: listProduct.sort_order,
    type: "product",
    description: template.description,
    variants: normalizeDemoVariants(template.variants),
    ...
    business: {
      ...
      cover_image: businessContext.media.cover_image,
...
function normalizeDemoVariants(variants: DemoProductTemplate["variants"] | undefined): ProductVariant[] {
  if (!variants) return [];
