...
import { ArrowLeft, BadgeCheck, ChevronRight, LoaderCircle, MapPin, SlidersHorizontal, Store, X } from "lucide-react";
...
import type { BusinessProduct, BusinessProductCategory, ProductListImage } from "../business.types";
...
export function BusinessProductsPage({ businessSlug, categorySlug }: { businessSlug: string; categorySlug?: string }) {
  ...
  const query = useInfiniteQuery({
    queryKey: ["business", businessSlug, "products", categorySlug, filterKey],
    initialPageParam: 1,
    queryFn: ({ pageParam }) => getBusinessProducts(businessSlug, { category: categorySlug, minPrice: appliedMin || undefined, maxPrice: appliedMax || undefined, isFeatured, sortBy, sortOrder, page: pageParam, pageSize: 20 }),
  ...
  const businessQuery = useQuery({
    queryKey: ["business", "detail", data?.business.slug],
    queryFn: () => getBusinessNameBySlug(data?.business.slug ?? ""),
    enabled: Boolean(data?.business.slug),
  });
  const demoData = query.isSuccess && products.length === 0 && businessQuery.data ? getDemoBusinessProductsPage(businessQuery.data, categorySlug) : null;
  ...
  function categoryHref(slug: string) {
    ...
    params.delete("modal");
    params.delete("page");
    const path = `/business/${encodeURIComponent(businessSlug)}/products/${encodeURIComponent(slug)}`;
  ...
  return <div className="min-h-dvh bg-white dark:bg-surface-dark">
    ...
    <main className="mx-auto w-full max-w-3xl bg-white pb-10 dark:bg-surface-dark">
      ...
      {displayProducts.length > 0 && <div className="grid grid-cols-2 gap-x-3 gap-y-5 px-page py-4">{displayProducts.map((product) => <article key={product.id} className="relative min-w-0"><SaveButton itemType="product" objectId={product.id} className="absolute right-2 top-2 z-10 flex size-9 items-center justify-center rounded-full bg-white/90 text-brand shadow-sm" /><Link href={`/product/${encodeURIComponent(product.slug)}`} onClick={rememberScrollPosition} className="block"><div className="relative aspect-square overflow-hidden rounded-xl border border-black/5 bg-surface-tertiary shadow-[0_1px_4px_rgba(15,23,42,0.06)] dark:border-white/10 dark:bg-surface-dark-tertiary dark:shadow-none"><Image src={productImage(product)} alt={product.name} fill sizes="(max-width: 768px) 50vw, 360px" className="object-cover transition-transform duration-200 hover:scale-[1.02]" />{product.is_featured && <span className="absolute left-2 top-2 inline-flex items-center gap-1 rounded-full bg-white/95 px-2 py-1 text-[10px] font-extrabold text-brand shadow-sm"><BadgeCheck size={11} aria-hidden="true" />Featured</span>}</div><h2 className="mt-2 line-clamp-2 text-base font-normal leading-5 text-foreground dark:text-foreground-dark">{product.name}</h2>{variantRows(product).length > 0 && <div className="mt-1 space-y-0.5">{variantRows(product).map((variant) => <p key={variant.name} className="truncate text-[11px] text-foreground-muted dark:text-foreground-dark-muted"><span className="font-bold">{variant.name}:</span> {variant.values.join(", ")}</p>)}</div>}<p className="mt-1 text-xs font-extrabold text-foreground dark:text-foreground-dark">{displayPrice(product.price, product.price_type, product.max_price)}</p></Link></article>)}</div>}
      <div ref={loadMoreRef} className="flex min-h-20 items-center justify-center px-page py-4 [overflow-anchor:none]" aria-live="polite">{query.isFetchingNextPage && <><LoaderCircle size={20} className="animate-spin text-brand" /><span className="ml-2 text-sm font-semibold text-foreground-muted">Loading more</span></>}{!query.hasNextPage && displayProducts.length > 0 && displayData && <MiniBusinessCard business={businessQuery.data} fallback={displayData.business} />}</div>
    </main>
...
function MiniBusinessCard({ business, fallback }: { business: Awaited<ReturnType<typeof getBusinessNameBySlug>> | undefined; fallback: { id: string; name: string; slug: string } }) {
  const address = business ? [business.location.locality, business.location.city.name].filter(Boolean).join(", ") : "";
  const image = business?.media.cover_image ?? business?.media.thumbnail;
  return <Link href={`/${encodeURIComponent(fallback.slug)}`} className="flex w-full items-center gap-3 rounded-xl border border-border-subtle bg-surface p-2.5 transition-colors hover:border-brand-200 hover:bg-surface-secondary dark:border-border-dark-subtle dark:bg-surface-dark-secondary dark:hover:border-brand-800">
    <div className="relative flex size-10 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-surface-tertiary dark:bg-surface-dark-tertiary">{image ? <Image src={image} alt="" fill sizes="40px" className="object-cover" /> : <Store size={18} className="text-foreground-muted" aria-hidden="true" />}</div>
    <div className="min-w-0 flex-1"><p className="truncate text-sm font-extrabold text-foreground dark:text-foreground-dark">{business?.name ?? fallback.name}</p>{address && <p className="mt-0.5 flex min-w-0 items-center gap-1 text-xs text-foreground-muted dark:text-foreground-dark-muted"><MapPin size={11} className="shrink-0" aria-hidden="true" /><span className="truncate">{address}</span></p>}</div>
