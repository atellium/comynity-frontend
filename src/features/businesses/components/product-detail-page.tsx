...
import { getBusinessNameBySlug, getProductBySlug, getProducts } from "../business.service";
...
export function ProductDetailPage({ slug }: { slug: string }) {
  ...
  const similarProductsQuery = useQuery({
    queryKey: ["products", "similar", similarCategory?.slug],
    queryFn: () => getProducts({ category: similarCategory?.slug, pageSize: 6 }),
    enabled: Boolean(!isDemoProduct && similarCategory?.slug),
  ...
  const businessImage = businessContactQuery.data?.media.cover_image ?? product.business.cover_image ?? businessContactQuery.data?.media.thumbnail ?? product.business.thumbnail;
  const hasBusiness = Boolean(product.business.slug);
  ...
  return <div className="min-h-dvh bg-white dark:bg-surface-dark">
    <main className="mx-auto max-w-3xl pb-32">
      ...
      {hasBusiness && <section className="mx-page mt-6 overflow-hidden rounded-xl border border-border-subtle bg-surface dark:border-border-dark-subtle dark:bg-surface-dark-secondary">
        <Link href={`/${encodeURIComponent(product.business.slug)}`} className="flex min-w-0 items-center gap-3 p-3 transition-colors hover:bg-surface-secondary dark:hover:bg-surface-dark-tertiary">
          <div className="relative size-11 shrink-0 overflow-hidden rounded-lg bg-surface-tertiary dark:bg-surface-dark-tertiary">{businessImage ? <Image src={businessImage} alt="" fill sizes="44px" className="object-cover" /> : <Store className="m-3 text-foreground-muted" size={20} />}</div>
          <div className="min-w-0 flex-1"><p className="truncate text-sm font-extrabold">{product.business.name}</p><p className="mt-1 flex min-w-0 items-center gap-1 text-xs text-foreground-muted"><MapPin size={12} className="shrink-0" aria-hidden="true" /><span className="min-w-0 truncate">{businessAddress}</span></p></div>
        ...
        <div className="grid grid-cols-3 border-t border-border-subtle dark:border-border-dark-subtle">
          <BusinessShortcut href={phoneNumber ? `tel:${phoneNumber}` : undefined} icon="fa-phone" label="Call" />
          <BusinessShortcut href={whatsappNumber ? `https://wa.me/${whatsappNumber}` : undefined} icon="fa-whatsapp" label="WhatsApp" external brandIcon />
          <BusinessShortcut href={directionsUrl || undefined} icon="fa-diamond-turn-right" label="Direction" external />
        </div>
      </section>}
    ...
    {hasBusiness && <div className="fixed inset-x-0 bottom-0 z-50 border-t border-border-subtle bg-white/95 px-page pb-[calc(env(safe-area-inset-bottom)+12px)] pt-3 shadow-[0_-6px_20px_rgba(0,0,0,0.08)] backdrop-blur dark:border-border-dark-subtle dark:bg-surface-dark/95"><div className="mx-auto grid max-w-3xl grid-cols-2 gap-3"><Link href={`/${encodeURIComponent(product.business.slug)}`} className="flex h-12 items-center justify-center rounded-xl border border-brand text-sm font-bold text-brand">Visit store</Link><button type="button" onClick={contactBusiness} disabled={!whatsappNumber && !phoneNumber} className="flex h-12 items-center justify-center gap-2 rounded-xl bg-brand text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-50">{whatsappNumber ? <i className="fa-brands fa-whatsapp text-lg" aria-hidden="true" /> : <Phone size={18} aria-hidden="true" />}{contactLabel}</button></div></div>}
...
function BusinessShortcut({ href, icon, label, external = false, brandIcon = false }: { href?: string; icon: string; label: string; external?: boolean; brandIcon?: boolean }) {
