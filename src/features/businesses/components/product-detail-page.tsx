...
import { ArrowLeft, BadgeCheck, ChevronRight, Handshake, MapPin, Share2, Store } from "lucide-react";
...
export function ProductDetailPage({ slug }: { slug: string }) {
  ...
  const productBusinessSlug = query.data?.business.slug;
  const similarProductsQuery = useQuery({
    queryKey: ["business", productBusinessSlug, "products", "similar", similarCategory?.slug],
    queryFn: () => getBusinessProducts(productBusinessSlug ?? "", { category: similarCategory?.slug, pageSize: 6 }),
    enabled: Boolean(!isDemoProduct && productBusinessSlug && similarCategory?.slug),
  ...
  const businessAddress = businessLocation
    ? [businessLocation.address, businessLocation.landmark, businessLocation.locality, businessLocation.city.name, businessLocation.city.state, businessLocation.postal_code]
        .filter((part): part is string => Boolean(part?.trim()))
        .filter((part, index, parts) => parts.indexOf(part) === index)
        .join(", ")
    : [product.business.locality, product.business.city.name].filter(Boolean).join(", ");
  const contactLabel = product.price_type === "ask" ? "Ask for price" : "More info";
  const contactHref = whatsappNumber
    ? `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(`Hello, can I get more info on this product: ${product.name}`)}`
    : phoneNumber
      ? `tel:${phoneNumber}`
      : "";
  const businessImage = businessContactQuery.data?.media.cover_image ?? product.business.cover_image ?? product.business.media?.cover_image ?? businessContactQuery.data?.media.thumbnail ?? product.business.thumbnail ?? product.business.media?.thumbnail;
  const businessSlug = businessContactQuery.data?.slug || product.business.slug;
  const businessName = businessContactQuery.data?.name || product.business.name;
  const hasBusiness = Boolean(businessSlug || businessName);

  async function shareProduct() {
    const url = window.location.href;
    const text = `Check out ${product.name} on Comynity:\n${url}`;
  ...
  return <div className="min-h-dvh bg-white dark:bg-surface-dark">
    <main className="mx-auto max-w-3xl pb-32">
      <div className="relative"><button type="button" onClick={() => router.back()} className="absolute left-3 top-[calc(env(safe-area-inset-top)+12px)] z-30 flex size-10 items-center justify-center rounded-full bg-white/90 text-foreground shadow-md backdrop-blur" aria-label="Go back"><ArrowLeft size={22} /></button><div className="absolute right-3 top-[calc(env(safe-area-inset-top)+12px)] z-30 flex gap-2"><button type="button" onClick={() => void shareProduct()} aria-label="Share product" className="flex size-10 items-center justify-center rounded-full bg-white/90 text-foreground shadow-md backdrop-blur"><Share2 size={20} aria-hidden="true" /></button><SaveButton itemType="product" objectId={product.id} className="flex size-10 items-center justify-center rounded-full bg-white/90 text-brand shadow-md backdrop-blur" /></div><ProductImageGallery images={product.images} productName={product.name} highlights={highlightedFields} /></div>
      ...
      <div className={`px-page ${product.images.length <= 1 ? "pt-3" : ""}`}>
        ...
        <div className="mt-1.5 flex flex-wrap items-baseline gap-2"><span className="text-sm font-extrabold text-brand">{displayPrice(product.price, product.max_price, product.price_type)}</span>{product.price_type === "fixed" && originalPrice && Number.isFinite(originalPrice) && <span className="text-xs font-semibold text-foreground-muted line-through">{money.format(originalPrice)}</span>}{product.price_type === "fixed" && discount > 0 && <span className="rounded-full bg-success-50 px-2 py-0.5 text-[10px] font-extrabold text-success-700">{discount}% off</span>}</div>
      ...
      {hasBusiness && <section className="mx-page mt-6" aria-label="Store">
        <BusinessCardLink href={businessSlug ? `/${encodeURIComponent(businessSlug)}` : undefined}>
          <div className="relative flex size-10 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-surface-tertiary dark:bg-surface-dark-tertiary">{businessImage ? <Image src={businessImage} alt="" fill sizes="40px" className="object-cover" /> : <Store size={18} className="text-foreground-muted" aria-hidden="true" />}</div>
          <div className="min-w-0 flex-1"><p className="truncate text-sm font-extrabold text-foreground dark:text-foreground-dark">{businessName || "Store"}</p><p className="mt-0.5 flex min-w-0 items-center gap-1 text-xs text-foreground-muted dark:text-foreground-dark-muted"><MapPin size={11} className="shrink-0" aria-hidden="true" /><span className="truncate">{businessAddress || "Store details"}</span></p></div>
...
function BusinessCardLink({ href, children }: { href?: string; children: React.ReactNode }) {
  const className = "flex w-full items-center gap-3 rounded-xl border border-border-subtle bg-surface p-2.5 transition-colors hover:border-brand-200 hover:bg-surface-secondary dark:border-border-dark-subtle dark:bg-surface-dark-secondary dark:hover:border-brand-800";
  if (!href) return <div className={className}>{children}</div>;
  return <Link href={href} className={className}>{children}</Link>;
}

function FixedProductAction({ href, icon, label, variant, external = false, brandIcon = false }: { href: string; icon: string; label: string; variant: "outline" | "solid"; external?: boolean; brandIcon?: boolean }) {
  const variants = {
    outline: "border border-brand bg-white text-brand dark:bg-surface-dark",
    solid: "bg-brand text-white",
  };
  const className = `flex h-12 items-center justify-center gap-2 rounded-xl text-sm font-bold ${variants[variant]} ${href ? "" : "pointer-events-none opacity-50"}`;
  return <a href={href || undefined} target={external && href ? "_blank" : undefined} rel={external && href ? "noreferrer" : undefined} aria-disabled={!href} className={className}><i className={`${brandIcon ? "fa-brands" : "fa-solid"} ${icon}`} aria-hidden="true" />{label}</a>;
}
